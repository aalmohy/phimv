function [F,C,Lnorm] = expm_frechet_cond(A)
%EXPM_FRECHET   Matrix exponential and its Frechet derivative.
%   [F,C,Lnorm] = EXPM(X) is the matrix exponential and an estimate C
%   of its condition number.  Lnorm is an estimate of the norm(L(A),1)
%   of the Frechet derivative, or more precisely the 1-norm of the
%   Kronecker form of L.

%   Based on EXPM: $Revision: 5.10.4.6 $  $Date: 2005/11/18 14:15:53 $
%   Started with expm_frechet and removed "L option".
...
if normA <= theta(end)
    ...
    for i = 1:length(m_vals)
        if normA <= theta(i)
           [F,Lnorm] = PadeApproximantOfDegree(m_vals(i));
%               normA, i, theta(i), mym = m_vals(i)
    ...
    Fpowers{1} = F;
    for i = 1:s
        Fpowers{i+1} = Fpowers{i}*Fpowers{i};           % Squaring
    end
    F = Fpowers{s+1};

    [temp,Lnorm] = funm_condest1(A,@expm,@fun_frechet13,1);

end

C = Lnorm*normA/norm(F,1);

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Nested function
    function L = fun_frechet13(flag,E)
    ...
    L = XU\(XL\(Lu+Lv + (Lu-Lv)*Fpowers{1}));

    for i = 1:s 
        L = Fpowers{i}*L + L*Fpowers{i};
    end
...
%%%%Nested Functions%%%%
    ...
    function [F,Lnorm] = PadeApproximantOfDegree(m)
        ...
        [temp,Lnorm] = funm_condest1(A,@expm,@fun_frechet,1);

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Nested function
        function L = fun_frechet(flag,E)
        % Frechet derivative of exponential.
